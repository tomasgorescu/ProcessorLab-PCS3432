...
do_irq_interrupt: @Rotina de interrupções IRQ
  ...
  STMFD sp!, {lr}			@armazena registrador lr na pilha de IRQ
  ...
  LDREQ lr, =linhaA @ lr = linhaA[0]

  CMP r10, #1  @ se proc 1 (B)
  LDREQ lr, =linhaB @ lr = linhaB[0]

  CMP r10, #2  @ se proc 2 (C)
  LDREQ lr, =linhaC @ lr = linhaC[0]
  
  LDMFD sp!, {r10-r11} @ restora r10 e r11 para erro original
  STMIA lr!, {r0-r12} @ armazena r1 a r12 e pc
  LDMFD sp!, {r1} @ r1 recebe o lr
  STMIA lr!, {r1} @ armazena lr (que esta em r1)
  MOV   r0, lr
  ...
  CMP r1, #2 @ se proc B
  LDREQ r0, =linhaC @ r0 = endereço de linhaB
...
stackC_init:
  ...
  MRS r0, cpsr @ salva cpsr corrente
  LDR r1, _taskC @ pc da func taskB definida em handler.c
  SUB r2, sp, #0x700 @ valor de sp para taskB
  ...
  MOV r4, #1
  STR r0, [r3] @ guarda r0
  STR r1, [r3, #4] @ guarda r1
  STR r2, [r3, #8] @ guarda r2
  STR r4, [r3, #12] @ guarda r3
  STR r0, [r3, #16] @ guarda r4
  STR r1, [r3, #20] @ guarda r5
  STR r2, [r3, #24] @ guarda r6
  STR r4, [r3, #28] @ guarda r7
  MOV pc, lr @ retorna
